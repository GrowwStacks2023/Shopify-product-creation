...
def read_config(config_path: str):
    ...
    try:
        ...
        raise
def create_product(store_url: str, product_data: dict, access_token: str, year: str, quarter: str, config: dict):
    # Update title to include quarter and year
    product_data['product']['title'] = f"Q{quarter} {year} - {product_data['product'].get('title', '')}"
    
    # Set additional product details
    product_data['product'].update({
        'published_scope': "web",
        'product_category': "Brochures in Advertising & Marketing",
        'product_type': "MarketCompass Report",
        'vendor': "Barrett HTL"
    })
    
    # Ensure variants key exists and is a list
    if 'variants' not in product_data['product'] or not isinstance(product_data['product']['variants'], list) or not product_data['product']['variants']:
        # Create a default variant using Price and CompareToPrice from config
        product_data['product']['variants'] = [{
            'price': config.get('Price', 0.00),
            'compare_at_price': config.get('CompareToPrice', 0.00),
...
def process_pdfs(source_folder: str, store_url: str, access_token: str, config: dict, year: str, quarter: str, markets_to_process: Optional[int] = None):
    ...
    for pdf_file in pdf_files:
        ...
        try:
            ...
            product_response = create_product(store_url, product_data, access_token, year, quarter, config)
...
def send_file_and_rename(csv_file_path):
    """Send and rename file, return new file path"""
    # Send the file to the hook
    send_csv_to_webhook(csv_file_path, "https://hook.eu1.make.com/wdcdvyyfqli6rwhgj51jnu2d2yqrxpeg")

    # Rename the file after sending
    timestamp = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
    new_file_name = f"processed_{timestamp}.csv"
    new_file_path = os.path.join(os.path.dirname(csv_file_path), new_file_name)
    os.rename(csv_file_path, new_file_path)
    logging.info(f"Renamed CSV file to {new_file_name}")
    return new_file_path

def parse_args():
    parser = argparse.ArgumentParser(description='Process PDFs and create Shopify products')
    parser.add_argument(
        '--source-folder',
        default="/home/vyavasthapak/Desktop/Store/2024Q3",
        help='Path to source folder containing PDFs and images'
    )
    parser.add_argument(
        '--store-url',
        default="project-digital-shop.myshopify.com",
        help='Shopify store URL'
    )
    parser.add_argument(
        '--access-token',
        default="put your default access token here",
        help='Shopify access token'
    )
    parser.add_argument(
        '--credentials-path',
        default="/home/vyavasthapak/Desktop/Bradely/python-444308-6e5be7255eac.json",
        help='Path to Google Drive credentials JSON'
    )
...
def main():
    ...
    if os.path.exists(csv_file_path):
        ...
        # Get new file path after renaming
        new_csv_path = send_file_and_rename(csv_file_path)
        
        # Handle final action with new CSV path
        while True:
            ...
            if action in ['activate', 'delete', 'skip']:
                if action == 'activate':
                    activate_products(store_url, access_token, new_csv_path)
                elif action == 'delete':
